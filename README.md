...
npm run qa       # 交付门禁：类型 + 七层架构边界 + 核心行为 + 生产构建
```
...
分层依赖由 `npm run check:boundaries` 按 `docs/DEVELOPMENT.md` §3.3 对七层全部
文件强制执行：内核三层零 npm 包依赖，`ui` 对对局状态只读（白名单模块），
`audio`/`music` 不读对局、不影响战斗结果。
...
| `npm run check:boundaries` | 七层架构边界：越层依赖 / 浏览器 API / 非确定性随机源，名单外引用即失败 |
