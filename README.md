...
要点：单文件形态无任何外链（发布脚本内置体检，残留外链即失败）；相对路径 `base './'`，任意子目录托管可用。发布工具链跨平台（Node zip，Windows/macOS/Linux 一致）。存档（对局进度 / 每日挑战 / 偏好）保存在**玩家本机浏览器**的 localStorage，不随文件走；音乐在窗口失焦时自动静音，回到前台恢复。
...
| 装备 | 拖器匣器件到棋子身上（身上可合成的组件自动吞并）；组件拖到器匣内另一组件上直接合成；「卸 载」钮点选后点棋子，全身装备一键回器匣 |
...
| 命令 | 回答什么问题 |
|---|---|
| `npm run sim` | 九套预设流派对拍矩阵（双向平均）、综合胜率极差、先手偏差 |
...
| `npm run sim:big` | 大地域羁绊探针：墨门九守 / 兵家百战对九预设（122 条 / 对，双向） |
| `npm run sim:legend` | 三星五费天命专项：2★→3★ 镜像升级价值 + 天命包净贡献（队号与 uid 同翻镜像） |
| `npm run sim:match` | 整局模拟：局长、AI 原型名次差、三星产出、座位公平性 |
| `npm run sim:shop` | 商店出货概率核算：各星级追星次数中位数 |
