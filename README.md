...
要点：游戏零外部素材（视觉音频全程序化；羁绊小篆为 22 字内联子集），单文件形态无任何外链（发布脚本内置体检，残留外链即失败）；相对路径 `base './'`，任意子目录托管可用。存档（对局进度 / 每日挑战 / 偏好）保存在**玩家本机浏览器**的 localStorage，不随文件走。
...
├── render/    表现层：场景、棋子美术管线、调色板、特效、飘字、音频
...
| 轻素材管线 | 纸纹、颗粒、图标、音效全部程序化；棋子立绘为 64 张透明底 PNG，烘焙期程序合成 |
| DPR 渲染底座 | 画布缓冲 = 1920×1080 × K（K=min(dpr,2)），相机 zoom=K 锚点居中 —— 高分屏 1 逻辑 px = 1 设备 px，整屏不再拉伸发糊 |
...
棋子立绘为 64 张 AI 生成透明底 PNG（`render/pieceArt/ai/`），原生分辨率烘焙、星级银/金描边与墨兽罩染由管线程序派生；
