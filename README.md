...
8 组件 36 成品的全配方装备体系（任意两件组件必有合成）。单局约 25~40 分钟
（500 局实测平均约 32 回合 / 27 分钟，见 [docs/DESIGN.md §十二](./docs/DESIGN.md)）：
...
| 刷新 / 升级 / 锁店 | 底部操作区；`D` 刷新、`F` 买经验、`空格` 准备完毕。任何信息浮层（设置 / 调试台 / 羁绊全览 / 成员卡 / 侦查）打开时，键盘、点按拖拽与悬停详情**统一让路**——只挡按键不挡鼠标的"半穿透"不存在 |
...
- **技能是配置驱动**。行为 kind × 目标选择 × 参数组合出 64 个技能；
  文案由 `{atk}/{sp}/{threshold}/{dur}` 等模板键回填（护甲点数、减伤时长、灼烧伤害
  等平值/时长键一并覆盖）—— 数值与描述永不脱节（有测试钉住）。
...
**调参键白名单与实现同源由测试自动看守**：`traits.ts` 里每一个 `t('…')` 读取（含档位模板键）
都必须登记在 `TRAIT_TUNE_KEYS` 中，反之白名单不得出现死键——新增可调量漏登记、拼写错误
的覆盖路径都会在 CI 直接报红，扫描数据不会静默失真。
