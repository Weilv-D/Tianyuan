...
| 风险 | 测试文件 | 保护内容 |
|---|---|---|
| 战斗可信 | `determinism`、`battle-formula`、`grid`、`skills`、`traits-big`、`status-src` | 收敛、重演、战斗规则、占格、技能与大羁绊、叠层按源分计（外来层不挤占 maxStacks）与非法状态数值入表前即抛、治疗记账单计与技能暴击单骰（拆分段共享判定）、装备击杀钩子端到端打标、复活占位与领域入队即验（NaN 不软锁不部分提交） |
| 平衡锚点 | `balance`、`shop-odds` | 定档数值回归（CRN 断面关键锚）、商店概率表结构与 rollShop 行为契约 |
| 工具链可信 | `balance-tools` | 平衡工具链自身：补丁四类路径往返与嵌套键级回退、CRN 配对公式金锁、进程池与串行逐位一致（胜负矩阵与逐单位聚合 `units` 同在看守面 —— 聚合分发漂移而胜负恰好相同时只比胜率会漏检）、SQLite 往返、**调参键白名单与 traits.ts 读取双向同源**、src 不引用工具链 |
...
| 进度连续 | `save`、`match-resume`、`replay`、`undo`、`daily`、`status-src` | 存读档、恢复推进、回放、撤销（含随机流游标回滚与奇遇恩赐对称回写）、每日成绩、坏档单元清洗（丢弃/钳制/剥离，含装备超限截断、墨兽标记剥离、iid 重号去重、超长回池、恩赐验型，覆盖墨影快照与墨兽阵容）与配对兜底重掷不双记交手史、墨影空快照配对整表弃用、非墨兽行 b/ghost 双设整表弃用（与墨兽行必空同一互斥粒度）、玩家运行时字段清洗（streak/器匣/商店/交手史坏形状收敛到类型安全缺省）、墨影快照与实况同 iid 不被去重误杀（域独立去重）、恩赐清洗（空 options / 轮次脱钩置空） |
| 交互安全 | `hud-layout`、`input-coords`、`valid-placements`、`item-paging`、`robustness` | 单源几何（轨/日志/战报/弹窗/商店无重叠）、K=2 输入、拖拽落点合法域、器匣溢出分页的页数/钳制/绝对索引映射、满盘站位、坏输入与墨兽落地、moveToSlot 非整数槽位守卫、复活 spawn 在场账目与读档快照保留窗口、法力事件账本全通道（施法清空与复活回满入账）、偏好写盘返回值 |
