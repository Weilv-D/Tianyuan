...
| 进度连续 | `save`、`match-resume`、`replay`、`undo`、`daily`、`status-src` | 存读档、恢复推进、回放、撤销（含随机流游标回滚与奇遇恩赐对称回写）、每日成绩、坏档单元清洗（丢弃/钳制/剥离，含装备超限截断、墨兽标记剥离、iid 重号去重、超长回池、恩赐验型，覆盖墨影快照与墨兽阵容）与配对兜底重掷不双记交手史、墨影空快照配对整表弃用、墨影快照与实况同 iid 不被去重误杀（域独立去重）、恩赐清洗（空 options / 轮次脱钩置空） |
