 ...
 * 开战仍未选的由系统按同一选型纯函数代选并经同一入口发放（resolveHumanAdventure）
 * —— 不惩罚犹豫，恩赐也不无声蒸发。
 *
 * 纪律：恩赐全部走既有经济/装备/经验/上场系统，不新增资源种类；
 * 发放必须守恒（援军占用卡池、器匣物品计入持有）；文案规格口吻（每档绝对数值）。
 * 确定性契约：Rng 只由 Match 持有并按固定顺序消费（beginRound 内
 * rollAdventureOffer → 按 idx 顺序 grantAdventure），chooseAdventureIndex 是
 * 纯函数、不消费随机 —— 打乱任一顺序都会让旧存档/回放的 rng 流错位。
 *
