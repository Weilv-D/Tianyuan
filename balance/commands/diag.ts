...
const ai = requireIntArg(process.argv[2], '阵容下标 A', 0, 0, PRESET_COMPS.length); // 快攻压制
const bi = requireIntArg(process.argv[3], '阵容下标 B', 0, 1, PRESET_COMPS.length); // 后期大招
const seed = requireIntArg(process.argv[4], '种子', 1, 4242);
